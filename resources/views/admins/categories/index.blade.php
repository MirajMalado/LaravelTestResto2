...
<div class="col-lg-12 grid-margin stretch-card">
    <div class="card">
      <div class="card-body">
        <h4 class="card-title">Notre principale categorie</h4>
            ...
            <tbody>
                @foreach ($categories as $count => $category)
                    <tr>
                        ...
                        <td>{{$category->name}}</td>
                        <td>
                          <a href="{{route("category.show", $category->id)}}" class="btn btn-dark btn-rounded btn-icon">
                            <i class="mdi mdi-eye"></i>
                          </a>
                          <a href="{{route("category.edit", $category->id)}}" class="btn btn-warning btn-rounded btn-icon ml-2">
                              ...
                              <div class="modal-dialog modal-dialog-centered">
                                <div class="modal-content">
                                  <div class="modal-header">
                                    <h5 class="modal-title" id="deleteCategoryTitle{{$category->id}}">Supprimer <span class="text-primary">{{$category->name}}</span></h5>
                                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                  </div>
                                  <div class="modal-body">
                                    <p>Etes-vous sure de vouloir supprimer cette categorie de plat?</p>
                                  </div>
